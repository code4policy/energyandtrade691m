...
# Load the Excel data
data <- read_excel("Desktop/Book2.xlsx", sheet = "Sheet1")
data <- data %>%
  filter(`Annual Total` > 1949)
data
...
p <- ggplot(data, aes(x = Year, y = RenewableEnergy)) +
  ...
  scale_y_continuous(breaks = seq(200, 1200, by = 200), limits = c(0, 1200)) +
  labs(title = "Residential Renewable Energy Consumption: A Decline Followed by a Gradual Recovery",  # New headline
       subtitle = "U.S. Renewable Energy Consumption in Residential Sector (1950-2023)",# Subheadline,
       x = "Year", 
