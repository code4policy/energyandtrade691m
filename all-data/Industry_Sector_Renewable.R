...
# Load the Excel data
data <- read_excel("Desktop/Book3.xlsx", sheet = "Sheet1")
data
...
p <- ggplot(data, aes(x = Year, y = RenewableEnergy)) +
  ...
  scale_y_continuous(breaks = seq(500, 2600, by = 500), limits = c(0, 2600)) +
  labs(title = "Renewable Consumption in Industrial Sector: Steady Growth over Seven Decades",  # New headline
       subtitle = "U.S. Renewable Energy Consumption in Industrial Sector (1950-2023)",# Subheadline,
       x = "Year", 
